...
st.set_page_config(
    page_title="Healthcare NLP System",
    page_icon="🏥",
    layout="wide"
)
        ...
        with st.spinner("Analyzing text with BioBERT..."):
            try:
                ...
                if response.status_code == 200:
                    data = response.json()
                    st.success(f"Processed in {data.get('metadata', {}).get('processing_time_sec', 'N/A')} seconds")
                    entities = data.get('entities', [])
                    
                    if not entities:
                        st.info("No explicit medical entities confidently extracted from this text.")
                    else:
                        ...
                        for ent in entities:
                            ...
                            st.markdown(f"**{group}**: `{word}` (Confidence: {score:.2f})")
                else:
                    st.error(f"API Error {response.status_code}: {response.text}")
                    
            except Exception as e:
                st.error(f"Failed to connect to API Backend at {API_URL}. Is it running? {str(e)}")

with tab2:
    st.header("Query Ingested Patient Charts")
    st.markdown("Ask natural language questions against the stored patient charts vectorized using **FAISS**.")
    ...
    if st.button("Query Knowledge Base", key="rag_btn"):
        with st.spinner("Searching FAISS Index and generating response..."):
            try:
                # Default top k to 3
                response = requests.post(f"{API_URL}/api/v1/query", json={"query": query_input, "top_k": 3})
                
                if response.status_code == 200:
                    data = response.json()
                    st.success(f"Query returned in {data.get('processing_time_sec', 'N/A')} seconds")
